...
The first step is running Axe Core behind a Tor proxy. This will already anonymize all
...
port 9050 and only allows .onion nodes to connect:
	...
	./axed -onion=127.0.0.1:9050 -onlynet=onion -listen=0 -addnode=ssapp53tmftyjmjb.onion
...
your axed's P2P listen port (9937 by default).
	...
	-externalip=X   You can tell Axe Core about its publicly reachable address using
	                ...
	                /var/lib/tor/axecore-service/hostname. For connections
...
and open port 9937 on your firewall (or use port mapping, i.e., `-upnp` or `-natpmp`).
...
in `torrc` config file. For cookie authentication the user running axed must have read
...
which has the appropriate permissions. Before starting axed you will need to re-login
